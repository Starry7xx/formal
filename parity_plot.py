...
def obtain_preds_and_labels(pred, model, mapping_file_path, dft_target_path):
    ...
    ###修改的代码行36-43 加载完整的 DataFrame
    dft_df = pd.read_pickle(dft_target_path)

    # 警告：根据你之前的报错 (User 71)，ID 列是 'id'
    # 如果你的 oc20_val.pkl 文件里 ID 列是 'sid'，请下面用 'sid'

    dft_df = dft_df.set_index('id')  # <-- 使用 'id' 列作为索引
    ...
    for key, value in tqdm.tqdm(pred.items()):
        ...
        # 遍历所有预测值 (key 是 'random1390956', value 是预测的能量)
        for key, value in pred.items():
            sid = key  # key 本身就是 ID，不要转换

            # 检查这个 ID 是否存在于我们的 DFT 真实值 DataFrame 中
            if sid in dft_df.index:
                # 如果存在，就同时保存预测值和真实值
                preds.append(value)
                labels.append(dft_df.loc[sid, 'target'])  # <-- 从 dft_df 中获取 'target' 列

        # (确保你删掉了原来在循环外的 preds.append 和 labels.append 行)
...
def parity_plot(label, pred, 
    ...
    lims = xylim #[min(min(label), min(pred)) - 0.6, max(max(label), max(pred)) + 0.6]
    # lims = xylim
